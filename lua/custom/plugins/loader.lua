-- Carica dinamicamente tutti i file di plugin .lua in questa directory
local plugins = {}

-- Percorso relativo alla directory dei plugin dalla root della configurazione di nvim
local plugins_path = "lua/custom/plugins/spec"
local plugins_module_path = "custom.plugins.spec"
...
for _, file_name in ipairs(vim.fn.readdir(full_path)) do
  -- Controlla se è un file .lua e non è questo file (init.lua)
